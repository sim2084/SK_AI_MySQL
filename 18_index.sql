...
INSERT INTO phone
VALUES
(1,'galaxyS23',1300000),(2,'galaxyZfold',1400000),(3,'iphone15pro',1500000);

-- phone_name(인덱스 없는 컴럼) 기준 조회 실행 결과
...
-- phone_name(인덱스가 생성된 컴럼) 기준 조회 실행 결과
-- type : ref, possible keys : idx_name => 방금 생성한 index를 참조하여 실행
-- rows : 1 => index가 관리하는 포인터를 통해 바로 뎅터 조회
EXPLAIN
SELECT
    *
FROM
...
-- 복합 인덱스 생성 (주로 첫 번째 컬럼 검색에 더 많이 사용됨)
